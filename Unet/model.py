    ...
    def __init__(self, input_img, nb_classes, scope='train'):
        ...
        with tf.variable_scope(self.scope):
            ...
            with tf.variable_scope('down1'):
                self.conv1 = tf.layers.Conv2D(filters = 64, kernel_size = (3,3), strides = 1,
                                              padding = 'same', name = 'conv1')(self.input)
                self.activation1 = tf.nn.relu(self.conv1)
                self.conv2 = tf.layers.Conv2D(filters = 64, kernel_size = (3,3),
                                              padding = 'same', strides = 1, name = 'conv2')(self.activation1)
            ...
            with tf.variable_scope('down2'):
                self.conv3 = tf.layers.Conv2D(filters = 128, kernel_size = (3,3), strides = 1,
                                              padding = 'same', name = 'conv3')(self.maxpool1)
                self.activation3 = tf.nn.relu(self.conv3)
                self.conv4 = tf.layers.Conv2D(filters = 128, kernel_size = (3,3), strides = 1,
                                              padding = 'same', name = 'conv4')(self.activation3)
            ...
            with tf.variable_scope('down4'):
                self.conv7 = tf.layers.Conv2D(filters = 512, kernel_size = (3,3), strides = 1,
                                              padding = 'same', name = 'conv7')(self.maxpool3)
                self.activation7 = tf.nn.relu(self.conv7)
                self.conv8 = tf.layers.Conv2D(filters = 512, kernel_size = (3,3), strides = 1,
                                              padding = 'same', name = 'conv8')(self.activation7)
                self.activation8 = tf.nn.relu(self.conv8)
