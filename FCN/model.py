...
class FCN:
    ...
    def _conv_layer(self, bottom, name, shape):
        with tf.variable_scope(name) as scope:
            filt = self.get_conv_filter(name, shape)
            conv = tf.nn.conv2d(bottom, filt, [1, 1, 1, 1], padding='SAME')

            #conv_biases = self.get_bias(name, shape)
    ...
    def _fc_layer(self, bottom, name, num_classes=None,
        ...
        with tf.variable_scope(name) as scope:
            ...
            conv = tf.nn.conv2d(bottom, filt, [1, 1, 1, 1], padding='SAME')
            #conv_biases = self.get_bias(name, shape, num_classes=num_classes)
            #bias = tf.nn.bias_add(conv, conv_biases)
    ...
    def get_conv_filter(self, name, shape):
        init = tf.constant_initializer(value=np.zeros(shape),
                                       dtype=tf.float32)
    ...
    def get_bias(self, name, shape, num_classes = None):
        if name == 'fc8':
            bias_wights = self._bias_reshape(np.zeros(shape), shape[0],
                                             num_classes)
            shape = [num_classes]
        init = tf.constant_initializer(value=np.zeros(shape),
                                       dtype=tf.float32)
    ...
    def __init__(self,input_img,nb_classes,scope='train'):
        ...
        with tf.variable_scope(self.scope, reuse=tf.AUTO_REUSE):
          ...
          self.conv1_1 = self._conv_layer(self.input, "conv1_1",[3,3,3,64])
          ...
          self.pred_up = softmax(self.upscore32)
